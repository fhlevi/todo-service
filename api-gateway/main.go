...
func main() {
	todoServiceURL = getEnv("TODO_SERVICE_URL", "http://localhost:3001")
	...
	// Proxy routes
	r.Any("/api/todo/*path", proxyHandler(todoServiceURL))
...
func proxyHandler(targetURL string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if strings.HasPrefix(originalPath, "/api/todo") {
			path = strings.TrimPrefix(originalPath, "/api/todo")
		}
